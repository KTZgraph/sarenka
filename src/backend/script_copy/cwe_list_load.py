...
def unzip(zip_filename: str) -> str:
...
def parse(extracted_file_name: str) -> list:
    cwe_list = []
    ...
    for cwe in weakness:
        ...
        cwe_obj["description"] = (
            cwe.getElementsByTagName("Description")[0].childNodes[0].data
        )

        try:
            cwe_obj["extended_description"] = (
                cwe.getElementsByTagName("Extended_Description")[0].childNodes[0].data
            )
